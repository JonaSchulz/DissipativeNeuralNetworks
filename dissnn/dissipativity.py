...
class NonlinearOscillator3NodeDynamics:
    def __init__(self, alpha=1, beta=1, k=1, **kwargs):
        self.alpha = alpha
        self.beta = beta
        self.k = k
    ...
    def compute_u(self, x, adjacency_matrix):
        """
        Compute the control input u for the dissipativity evaluation (u_i = sum_j A_ij * (x_j[2]^2 - x_i[2]^2))
        ...
        u_values = []
        for t in range(x.shape[0]):
            u_t = -torch.sum(adjacency_matrix * (x[t, :, 1].reshape(-1, 1)**2 - x[t, :, 1].reshape(1, -1)**2), dim=1)
            u_values.append(u_t)
        return torch.stack(u_values).unsqueeze(-1)
